...
import { carsSelectors, categorySelectors } from '../../redux/selectors';
...
function UpdateCarsPage(props) {
    const { categoryData, getCategoryData, createCar, getCarsData, updateCarsData, dataUpdation } = props;
    const navigation = useNavigate();
    const formik = useFormik({
        enableReinitialize: true,
        initialValues: {
            id: dataUpdation?.id,
            carName: dataUpdation?.name,
            Make: dataUpdation?.make,
            Model: dataUpdation?.model,
            Color: dataUpdation?.color,
            RegNo: dataUpdation?.regNo,
            Category: dataUpdation?.category
        },
        validationSchema: validationSchema,
        onSubmit: (values) => {
            //alert(JSON.stringify(values, null, 2));
            updateCarsData(values)
            navigation('/cars')
        },
    });
...
const mapStateToProps = createStructuredSelector({
    ...
    dataUpdation: carsSelectors.makeSelectShowCarsUpdation(),
});
const mapDispatchToProps = function (dispatch, ownProps) {
    return {
        ...
        updateCarsData: (data) => {
            dispatch(carsAction.updatingCarsData(data));
        },
    };
};
export default connect(mapStateToProps, mapDispatchToProps)(UpdateCarsPage);
