...
import { Grid, Typography, Container, TableHead, Button } from '@mui/material';
...
function CategoryPage(props) {
    const { getCategoryData, categoryData, deleteCategoryData,updateCategoryData } = props;
    const navigation = useNavigate();
    React.useEffect(() => {
        getCategoryData();
    }, [])
    const rows = categoryData?.sort((a, b) => (a.id < b.id ? -1 : 1));
        ...
        <>
            {categoryData !== null && (<>
                ...
                <Grid item xs={12} align="right" style={{ paddingRight: 25 }}>
                    <Button variant="contained" onClick={(event) => { event.preventDefault(); navigation('/addCategory') }}><AddIcon />Create</Button>
                </Grid>
                <br />
                                    ...
                                    <TableRow key={row.name}>
                                        ...
                                        <TableCell style={{ width: 160 }} align='center'>
                                            <IconButton onClick={(event) => {
                                                ...
                                                updateCategoryData(row.id)
                                                navigation('/updateCategory')
                                            }}>
                                                <UpdateIcon />
                                            </IconButton>
                                            <IconButton onClick={(event) => {
...
const mapDispatchToProps = function (dispatch, ownProps) {
    return {
        ...
        deleteCategoryData: (data) => {
            dispatch(categoryAction.deleteCategory(data));
        },
